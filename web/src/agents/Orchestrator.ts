...
export class Orchestrator {
  ...
  private setAgentStatus(id: string, status: AgentStatus, output?: any, input?: any) {
    this.state.agents[id] = { ...this.state.agents[id], status, output, ...(input !== undefined ? { input } : {}) };
  ...
  async processUrl(url: string, youtubeKey: string, geminiKey: string, mockMode: boolean = false) {
    ...
    Object.keys(this.state.agents).forEach(key => {
      ...
      this.state.agents[key].input = undefined;
    ...
    this.setAgentStatus('data_collector', 'running', undefined, { video_url: url });
    ...
    if (!geminiKey) {
      ...
      deconstructorPromise = dataCollectorPromise.then(() => {
        this.setAgentStatus('deconstructor', 'running', undefined, {
