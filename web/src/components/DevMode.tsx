  ...
  return (
    <button type="button" className={`agent-card ${agent.status} ${isSelected ? 'selected' : ''}`} onClick={onSelect}>
      <div className="agent-header">
        <Icon size={18} style={{ color: agent.status === 'running' ? 'var(--studio-grad-start)' : 'inherit' }} />
        <span style={{ flex: 1 }}>{agent.name}</span>
        <span className="agent-status">{agent.status}</span>
      </div>
      {agent.status === 'running' && (
        <div style={{ color: 'var(--studio-mute)', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Activity size={14} className="animate-pulse-glow" style={{ color: 'var(--studio-grad-start)' }} /> Processing...
        </div>
      )}
...
export const DevMode: React.FC<DevModeProps> = ({ isOpen, onClose, state, geminiKey, youtubeKey, inline = false }) => {
  ...
  const inspectorModal = activeTab === 'diagram' && inspectorAgentId && selectedAgent && (
    <div className="modal-overlay">
      <div className="modal-content" style={{ maxWidth: '880px' }}>
        ...
        <p style={{ color: 'var(--studio-mute)', marginBottom: '1rem', fontSize: '13px' }}>
        ...
        <div className="inspector-section">
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.75rem' }}>
            <button type="button" className="btn-gradient" onClick={() => rerunSelectedAgent(selectedEdit?.editedInput || '')} disabled={!selectedEdit?.editedInput || selectedEdit?.isLoading}>
              {selectedEdit?.isLoading ? 'Running...' : 'Run Agent with Test Input'}
            </button>
          </div>
          {selectedEdit?.error && <p style={{ color: 'var(--error)', marginTop: '0.5rem', fontSize: '13px' }}>{selectedEdit.error}</p>}
        </div>
        <div className="inspector-section">
  ...
  return (
    <div className={`dev-overlay ${inline ? 'inline' : isOpen ? 'open' : ''}`}>
      <div className="studio-page-header dev-page-header">
        <div className="dev-page-header-text">
          <h2 className="studio-page-title">Agents</h2>
          <p className="studio-disclaimer">
            Real-time view of the multi-agent orchestration engine behind every analysis.
          </p>
        </div>
        {!inline && (
          <button className="btn-icon" onClick={onClose} style={{ color: 'var(--studio-ink)' }}>
            <X size={28} />
          </button>
        )}
      </div>
      ...
      {activeTab === 'diagram' ? (
        !state ? (
          <div className="dev-empty-state">
            ...
            <p className="dev-empty-title">Awaiting trigger event</p>
            <p className="dev-empty-sub">Run an analysis from the dashboard to watch the agents work.</p>
          </div>
        ) : (
          ...
          <div className="config-details">
            <div className="inspector-section">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button type="button" className="studio-see-more dev-copy-btn" onClick={() => navigator.clipboard.writeText(selectedAgentConfig.prompt)}>
                  <Copy size={14} /> Copy
                </button>
              </div>
      ...
      {inspectorModal && typeof document !== 'undefined' ? createPortal(inspectorModal, document.body) : null}
    </div>
  );
};
