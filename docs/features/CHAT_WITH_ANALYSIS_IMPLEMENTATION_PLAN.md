# Chat with Analysis — Implementation Plan

**Status:** In progress. Entry button, chat panel, prompt, packet builder, and `/api/gemini-chat` are in. Chat UI is a working placeholder — restyle from Figma next.

This feature lets a creator open a conversation on a finished analysis and ask follow-up questions. A new **Chat with Analysis** agent answers from that analysis (and optional past analyses). It is not a new step in the run-analysis pipeline.

---

## 1. Goal

On the home dashboard, when a current analysis exists, show a **Chat with Analysis** button. It opens a chat where the user can ask things like:
...
Chat with Analysis is a **seventh agent**, conversational, user-triggered:
...
Also add a client helper in `llm.ts`, e.g. `generateText` / `runChatWithAnalysisAgent`, using the same queue (`MIN_GEMINI_INTERVAL_MS = 2000`) and the same rule: browser key if set, otherwise the server proxy.
...
- **Entry (done — visual only):** `Chat with Analysis` pill at the top of the right-hand skills column in `Dashboard.tsx`, above AI diagnosis. Matches Figma node `485:1205` (`AskAnalysisBtn`): height 34px, radius 17px, background `#efefef`, 13px medium, Lucide filled `Sparkles` at 18px. Class: `.studio-chat-analysis-btn`. Hidden automatically because `Dashboard` only renders when there is analysis (`!skillData` returns null; App only mounts Dashboard when `activeSkill` exists). Do not show on the empty “No current analysis” state.
- **Wired:** the button opens `ChatWithAnalysisPanel`. UI is a working placeholder until Figma lands.
- **Surface:** modal using existing overlay chrome (22px radius, close X, black Send). Restyle from Figma without changing the agent.
...
- **Past Analysis:** same button when a history row is open (`selectedHistory`), scoped to that record. Already true because history reopen uses the same `Dashboard`.
...
- Agent-to-agent handoff (Chat with Analysis does not call Coach again)
- Editing / re-running the original analysis from chat
- Admin transcript viewer

---

## 4. System instructions (Chat with Analysis agent)

Add this to `web/src/agents/prompts.ts` as `CHAT_WITH_ANALYSIS_PROMPT`, and mirror it in `SYSTEM_OVERVIEW.md` / `docs/system_design/02_AGENTS.md` the same way the other agents are documented.
...
| File | Change |
|---|---|
| `web/src/agents/prompts.ts` | Add `CHAT_WITH_ANALYSIS_PROMPT` |
| `web/src/utils/llm.ts` | `runChatWithAnalysisAgent(apiKey, packet, messages)` + text generate helper |
| `api/gemini-chat.mjs` | New text / multi-turn proxy (keep `api/gemini.mjs` JSON-only) |
| `web/src/components/ChatWithAnalysisPanel.tsx` | Modal/drawer + thread + composer |
| `web/src/index.css` | Chat layout + entry button (`.studio-chat-analysis-btn`) |
| `web/src/components/Dashboard.tsx` | Entry button wired to `onChatWithAnalysis` |
...
2. Clicks **Chat with Analysis** (pill above AI diagnosis).
3. Panel opens with empty state + optional chips.
4. User sends a question.
5. Client builds packet + thread, calls `runChatWithAnalysisAgent`.
...
Prompt, packet builder, `/api/gemini-chat`, `runChatWithAnalysisAgent`, a minimal modal that can send one question and show a reply.

**Phase B — Conversation UI**  
**Entry button + working chat panel are in** (placeholder styling). Remaining: restyle from Figma.
...
## 10. Open decisions (need a yes before coding the rest)

1. **Button label:** decided — **Chat with Analysis** (Figma + live dashboard).
...
The dashboard entry button, chat panel, prompt, packet builder, and `/api/gemini-chat` are in. Remaining: restyle the panel from Figma, then optional persistence.
